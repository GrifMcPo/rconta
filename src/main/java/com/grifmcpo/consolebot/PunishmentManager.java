    ...
    public List<String> getBanList(int page, int pageSize) {
        ...
        for (String key : punishmentConfig.getKeys(false)) {
            if (punishmentConfig.getBoolean(key + ".bans.active", false)) {
                bans.add("• " + key + " → " + punishmentConfig.getString(key + ".bans.reason", "Без причины") +
                        " (выдал: " + punishmentConfig.getString(key + ".bans.issuer", "Unknown") +
                        " | " + formatDate(punishmentConfig.getLong(key + ".bans.timestamp", 0)) + ")");
            }
        }
    ...
    public List<String> getMuteList(int page, int pageSize) {
        ...
        for (String key : punishmentConfig.getKeys(false)) {
            if (punishmentConfig.getBoolean(key + ".mutes.active", false)) {
                mutes.add("• " + key + " → " + punishmentConfig.getString(key + ".mutes.reason", "Без причины") +
                        " (выдал: " + punishmentConfig.getString(key + ".mutes.issuer", "Unknown") +
                        " | " + formatDate(punishmentConfig.getLong(key + ".mutes.timestamp", 0)) + ")");
            }
        }
