...
public class CommandListener implements Listener {
    ...
    // ===== ВСЕ КОМАНДЫ, КОТОРЫЕ БЛОКИРУЕМ (чтобы не мешали) =====
    private final String[] BLOCKED_COMMANDS = {
        // Команды банов
        "/ban", "/tempban", "/unban", "/ban-ip", "/ipban", "/unbanip",
        // Команды мутов
        "/mute", "/tempmute", "/unmute", "/globalmute", "/unmute",
        // Команды киков
        "/kick", "/kickall",
        // Предупреждения
        "/warn", "/unwarn", "/warns",
    ...
    public void onPlayerCommand(PlayerCommandPreprocessEvent event) {
        ...
        // =========================================================
        // ==== 1. БЛОКИРУЕМ ВСЕ ЧУЖИЕ КОМАНДЫ НАКАЗАНИЙ =====
        // =========================================================
        for (String blocked : BLOCKED_COMMANDS) {
            // Блокируем команду, если она начинается с заблокированной
            if (command.startsWith(blocked) || 
                command.startsWith("essentials:" + blocked) ||
                command.startsWith("flectonepulse:" + blocked) ||
                command.startsWith("punisherx:" + blocked) ||
                command.startsWith("lp:" + blocked)) {
                
                event.setCancelled(true);
                player.sendMessage("§c⛔ Эта команда отключена!");
                player.sendMessage("§7Используйте наши команды:");
                player.sendMessage("§7/ban, /mute, /kick, /unban, /unmute");
                return;
            }
        }
